...
pipePath="/tmp/chatbox_pipe_$$"
mkfifo "$pipePath"

# Starts Zenity IN THE BACKGROUND, as the user, with the full environment
if [[ "$function" == "install" ]]; then
  zenityTitle=$"Hermes Agent Deskto Install"
  zenityText=$"Installing Hermes Agent Deskto, Please wait..."
else
  zenityTitle=$"Hermes Agent Deskto Uninstall"
  zenityText=$"Uninstalling Hermes Agent Deskto, Please wait..."
fi
...
updateTask() {
  if [[ "$function" == "install" ]]; then
    pacman -Syu --noconfirm hermes-agent-desktop
  else
    pacman -Rcs --noconfirm hermes-agent-desktop
  fi
  exitCode=$?
}
updateTask > "$pipePath"
...
if [[ "$exitCode" == "0" ]] && [[ "$function" == "install" ]]; then
  zenityText=$"Hermes Agent Deskto installed successfully!"
  runAsUser "zenity --info --text=\"$zenityText\""
else
  zenityText=$"Failed to install Hermes Agent Deskto!"
