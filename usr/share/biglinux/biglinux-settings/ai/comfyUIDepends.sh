...
pipePath="/tmp/comfyDepends_pipe_$$"
mkfifo "$pipePath"

# Starts Zenity IN THE BACKGROUND, as the user, with the full environment
if [[ "$function" == "install" ]]; then
  zenityTitle=$"comfyUI Depends Install"
  zenityText=$"Installing comfyUI Depends, Please wait..."
fi
...
if [[ "$exitCode" == "0" ]] && [[ "$function" == "install" ]]; then
  zenityText=$"comfyUI Depends installed successfully!"
  runAsUser "zenity --info --text=\"$zenityText\""
else
  zenityText=$"Failed to install comfyUI Depends!"
