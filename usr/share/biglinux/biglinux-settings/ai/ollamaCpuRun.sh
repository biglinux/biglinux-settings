...
if [[ "$function" == "install" ]]; then
  ...
  zenityText=$"Uninstalling Ollama, Please wait..."
fi
...
updateTask() {
  if [[ "$function" == "install" ]]; then
    pacman -Syu --noconfirm ollama
    systemctl enable --now ollama.service
  else
    systemctl disable --now ollama.service
    pacman -Rcs --noconfirm ollama
  fi
  exitCode=$?
}
updateTask > "$pipePath"
...
if [[ "$exitCode" == "0" ]] && [[ "$function" == "install" ]]; then
  zenityText=$"Ollama installed successfully!"
  runAsUser "zenity --info --text=\"$zenityText\""
else
  zenityText=$"Failed to install Ollama!"
